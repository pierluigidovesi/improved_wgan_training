...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    for device_index, (device, real_data_conv) in enumerate(zip(DEVICES, split_real_data_conv)):
        ...
        with tf.device(device):
            ...
            # cast real_data_con as tf.float32
            # div by 255 - > data in 0,1
            # - 0.5 to put mean in 0
            # mult by 2 to have data in -1,1
            # reshape data into: rows:BATCH_SIZE/DEVICE, columns: OUTPUT_DIM (64x64x3)
            real_data = tf.reshape(2 * ((tf.cast(real_data_conv, tf.float32) / 255.) - .5),
                                   [BATCH_SIZE / len(DEVICES), OUTPUT_DIM])

            # give number of requested images
            # take fake data from Generator
            fake_data = Generator(BATCH_SIZE / len(DEVICES))

            # prob given by Discriminator -> 0 Fake, 1 Real

            # prob given by Discriminator on real_data
            disc_real = Discriminator(real_data)
            # prob given by Discriminator on fake_data
            disc_fake = Discriminator(fake_data)

            if MODE == 'wgan':
                # for GENERATOR: take reduce_mean of fake prob
                gen_cost = -tf.reduce_mean(disc_fake)
                # for DISCRIMINATOR: take reduce_mean fake - reduce_mean real
            ...
            # ACTIVE NOW
                ...
                # img by img get pixel differences
                differences = fake_data - real_data

                # add to real_data the differences*random_alpha
                interpolates = real_data + (alpha * differences)

                # backprop on gradients of Discriminator(interpolates)
                # TODO what's happening here?
                gradients = tf.gradients(Discriminator(interpolates), [interpolates])[0]

                # L2 norm respect to columns (trust)
                ...
                try:  # tf pre-1.0 (bottom) vs 1.0 (top)
                    ...
                    # label of gen is always one. it means that discriminator classify as real the fakes.
                    gen_cost = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_fake,
                                                                                      labels=tf.ones_like(disc_fake)))

                    # label of fakes for discriminator is always zero.
                    # it means that the discriminator classify as fake the fake.
                    disc_cost = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_fake,
                                                                                       labels=tf.zeros_like(disc_fake)))

                    # label of real for discriminator is always one.
                    # it means that the discriminator classify as real the real.
                ...
                # here same stuff, we use LEAST SQUARE
    ...
    if MODE == 'wgan':
        ...
        disc_train_op = tf.train.RMSPropOptimizer(learning_rate=5e-5).minimize(disc_cost,
                                                                               var_list=lib.params_with_name(
                                                                                   'Discriminator.'),
                                                                               colocate_gradients_with_ops=True)
        # for classic-WGAN: weight clipping
        clip_ops = []
